...
/**
 * @Route("/admin/abonne")
 * Toutes les routes de ce contrôleur vont commencer par "/admin/abonne"
 */
class AbonneController extends AbstractController
{
    ...
    /**
     * REGEX: \d veut dire que le caractère doit être une chiffre
     *         + pour dire que le caractère qui précède peut être présent une ou plusieurs fois
    ...
    */
    public function nouveau(Request $request, EntityManagerInterface $em, Encoder $encoder){
        $abonne = new Abonne;
        // La méthode 'CreateForm' va créer un objet qui va permettre de gérer un formulaire créé à partir de la classe Form\AbonneType. On lie ce formulaire à l'objet $abonne
        // AbonneType::class = "App\Form\AbonneType", c'est à dire le nom complet de la classe en string
        $form = $this->createForm(AbonneType::class, $abonne);
        
        // $form va gérer les données du formulaire encvoyées par la requête HTTP
        $form->handleRequest($request);
